...
async def handle_adm_delete_type(update: Update, context: ContextTypes.DEFAULT_TYPE, params=None):
    ...
    try:
        ...
        if product_count > 0 or reseller_discount_count > 0:
            ...
            # <<< MODIFICATION START >>>
            force_delete_msg = (
                f"⚠️ Type '{type_name}' is currently used by {usage_details}.\n\n"
            ...
            keyboard = [
                [InlineKeyboardButton(f"💣 Force Delete Type & {usage_details}", callback_data=f"confirm_force_delete_type|{type_name}")], # New callback
            ...
            context.user_data["confirm_action"] = f"delete_type|{type_name}" # Normal delete
...
# <<< NEW CALLBACK HANDLER FOR FORCE DELETE CONFIRMATION >>>
async def handle_confirm_force_delete_type(update: Update, context: ContextTypes.DEFAULT_TYPE, params=None):
    """Shows a final, more severe confirmation for force deleting a product type and its associated items."""
    query = update.callback_query
    if query.from_user.id != ADMIN_ID: return await query.answer("Access Denied.", show_alert=True)
    if not params or len(params) < 1:
        await query.answer("Error: Type name missing for force delete.", show_alert=True); return

    type_name = params[0]
    context.user_data["confirm_action"] = f"force_delete_type_CASCADE|{type_name}" # New specific action
...
async def handle_confirm_yes(update: Update, context: ContextTypes.DEFAULT_TYPE, params=None):
    ...
    try:
        ...
        # --- Force Delete Product Type Logic (CASCADE) ---
            ...
            # 1. Get product IDs associated with this type to delete their media
            ...
            # 2. Delete from product_media for those products
            if product_ids_to_delete_media_for:
                ...
                # Schedule physical media directory deletions (async tasks)
                for pid in product_ids_to_delete_media_for:
                    media_dir_to_del = os.path.join(MEDIA_DIR, str(pid))
                    if await asyncio.to_thread(os.path.exists, media_dir_to_del): # Requires async
                        asyncio.create_task(asyncio.to_thread(shutil.rmtree, media_dir_to_del, ignore_errors=True))
                        logger.info(f"Force delete: Scheduled deletion of media dir: {media_dir_to_del}")

            # 3. Delete products of this type
            ...
            # 4. Delete reseller_discounts of this type
            ...
            # 5. Delete the product_type itself
            ...
            if delete_type_res.rowcount > 0:
                ...
                conn.rollback() # Should not happen if it existed before and was force deleted
                success_msg = f"❌ Error: Type '{type_name}' not found during final delete step. Partial changes might have occurred."
                next_callback = "adm_manage_types"
...
# --- New Handlers for Reassign Product Type ---
async def handle_adm_reassign_type_start(update: Update, context: ContextTypes.DEFAULT_TYPE, params=None):
    query = update.callback_query
    if query.from_user.id != ADMIN_ID: return await query.answer("Access Denied.", show_alert=True)
    # lang, lang_data = _get_lang_data(context) # Use helper if you have translations
...
async def _ask_for_reassign_new_type_name(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """Helper to ask for the new type name, used after old name is validated."""
    chat_id = update.effective_chat.id
    old_type_name = context.user_data.get('reassign_old_type_name')
    # lang, lang_data = _get_lang_data(context) # Uncomment if you use translations here

    if not old_type_name: # Should not happen if flow is correct
...
async def handle_adm_reassign_new_type_name_message(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if not old_type_name: # Should not happen
    ...
    keyboard = [[InlineKeyboardButton("✅ Yes, Reassign & Delete", callback_data="confirm_yes")],
                    [InlineKeyboardButton("❌ No, Cancel", callback_data="adm_manage_types")]] # Back to manage types
    await send_message_with_retry(context.bot, chat_id, msg, reply_markup=InlineKeyboardMarkup(keyboard), parse_mode=None)