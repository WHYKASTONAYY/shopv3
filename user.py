...
async def handle_single_item_discount_code_message(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    code_valid, validation_message, discount_details = validate_discount_code(entered_code, price_after_reseller_float)
    feedback_msg_template = ""
    ...
    if user_balance >= final_total_decimal:
        await send_message_with_retry(context.bot, chat_id, "⏳ Processing payment with balance...", parse_mode=None)
        ...
        context.user_data['basket_pay_snapshot'] = snapshot
